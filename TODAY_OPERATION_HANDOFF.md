...
- 现象：课程详情页先点 A 句子，再点 B 句子，会出现两个句子同时播放。
- 根因：
  - 每个 `SentenceCard` 独立维护播放状态，卡片之间没有共享“当前活跃播放器”；
  - 切到新卡片播放时，旧卡片的 mp3/TTS 不会被全局停止。
- 修复（已完成）：
  1. 新增句子卡片级全局播放互斥，同一时刻只允许 1 个卡片朗读；
  2. 新卡片开始播放时，会主动停止前一个卡片的音频与 TTS；
  3. 单句循环切换到其它卡片时，也会立即退出循环并复位状态。
- 如何验证：
  1. 打开任意 `/courses/[slug]`；
  2. 先点 A 句子“标准”，在未播完时立刻点 B 句子“标准”；
  3. 预期 A 立即停止，只剩 B 播放；
  4. 再验证 A 开启“单句循环”后点 B，也应只剩 B 播放。
- 关键文件：
  - `src/components/learning/sentence-card.tsx`
